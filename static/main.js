...
function evaluate(str) {
    ...
    let mas = str.split(' ').reverse();
    for (i=mas.length; i>=0; i--) {
        if (isOperation(mas[i])) {
            if (mas[i] == '+') mas[i] = Number(mas[i+2]) + Number(mas[i+1]);
            if (mas[i] == '-') mas[i] = Number(mas[i+2]) - Number(mas[i+1]);
            if (mas[i] == '*') mas[i] = Number(mas[i+2]) * Number(mas[i+1]);
            if (mas[i] == '/') mas[i] = Number(mas[i+2]) / Number(mas[i+1]);
            mas = mas.filter((v, idx) => idx!=i+1 && idx!=i+2);
        }
...
function clickHandler(event) {
    let sym = event.target.textContent;
    let btnType = event.target.className;
    console.log(btnType);
    if (btnType == 'key digit') {
        if ((screen.textContent == '0' || finish) && sym != '.') {
            screen.textContent = sym;
            finish = false;
        }
        else screen.textContent += sym;
    }
    else if (btnType == 'key operation') {
        screen.textContent += sym;
        finish = false;
    }
    else if (btnType == 'key bracket') {
        if (screen.textContent == '0' || finish) {
            screen.textContent = sym;
            finish = false;
        }
        else screen.textContent += sym;
    }
    else if (btnType == 'key result') {
        finish = true;
        screen.textContent = evaluate(compile(screen.textContent));
    }
    else if (btnType == 'key clear') {
...
window.onload = function () {
    btns.onclick = clickHandler;
}


var screen = document.getElementById('win');
var finish = false;
